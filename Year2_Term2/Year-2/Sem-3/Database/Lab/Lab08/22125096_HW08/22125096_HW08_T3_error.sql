...
begin
    ...
    if exists (
        ...
        begin
            ...
            from orders O join orderdetails OD on O.orderNumber = OD.orderNumber
            where YEAR(orderDate) = @year and MONTH(orderDate) = @month
            select @number_of_products = SUM(quantityOrdered), @total_revenue = SUM(P.buyPrice*OD.quantityOrdered), @total_retail_revenue = SUM(OD.priceEach*OD.quantityOrdered), @total_profit = @total_retail_revenue - @total_revenue
            from orders O
                join orderdetails OD on O.orderNumber = OD.orderNumber
                join products P on P.productCode = OD.productCode
            where YEAR(orderDate) = @year and MONTH(orderDate) = @month and O.status in ('Shipped', 'On Hold', 'In Process')
